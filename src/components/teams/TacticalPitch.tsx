...
function layoutPlayers(squad: Player[]): PitchPlayer[] {
  ...
  for (const [pos, players] of Object.entries(groups)) {
    ...
    players.forEach((p, i) => {
      ...
      result.push({
        id:          p.id,
        name:        p.name.split(' ').at(-1) ?? p.name,  // apellido
...
export function TacticalPitch({ squad, activeFilter, onFilterChange }: TacticalPitchProps) {
  ...
  return (
    ...
    >
      ...
      >
        {/* Borde de cancha */}
        <rect x="4" y="3" width="92" height="127" rx="1" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="0.6" />
        {/* Línea de medio */}
        <line x1="4" y1="66.5" x2="96" y2="66.5" stroke="rgba(255,255,255,0.18)" strokeWidth="0.5" />
        {/* Círculo central */}
        <circle cx="50" cy="66.5" r="12" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="0.5" />
        <circle cx="50" cy="66.5" r="0.8" fill="rgba(255,255,255,0.4)" />
        {/* Área grande superior */}
        <rect x="22" y="3" width="56" height="16" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="0.5" />
        {/* Área chica superior */}
        <rect x="36" y="3" width="28" height="6" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="0.5" />
        {/* Área grande inferior */}
        <rect x="22" y="114" width="56" height="16" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="0.5" />
        {/* Área chica inferior */}
        <rect x="36" y="124" width="28" height="6" fill="none" stroke="rgba(255,255,255,0.18)" strokeWidth="0.5" />
      </svg>

      {/* Nodos de jugadores */}
      {nodes.map((node) => {
        const isActive = activeFilter === 'ALL' || activeFilter === node.posLabel;
        const style = NODE_STYLE[node.posLabel] ?? NODE_STYLE.MED;
        return (
          <button
            key={node.id}
            onClick={() =>
              onFilterChange(activeFilter === node.posLabel ? 'ALL' : node.posLabel)
            }
